...
!-----------------------------------------------------------------------
 character(len=128) :: version =  '$Id: bgrid_vert.F90,v 19.0 2012/01/06 19:55:18 fms Exp $'
...
!-----------------------------------------------------------------------
integer :: k, nlev, lverbose, logunit
real    :: rgog
...
!--- optional output of coordinate values ----
   ...
   if (mpp_pe() == mpp_root_pe()) then
       if (lverbose > 0) then
           write (logunit,*) 'Number of vertical levels =', Vgrid%nlev
           write (logunit,*) 'Approxiamte model level locations in pascals:'
           write (logunit,*) '  Full levels = ', Vgrid%pfull
           write (logunit,*) '  Half levels = ', Vgrid%phalf
           if (Vgrid%hybrid) &
           write (logunit,*) 'Hybrid coordinate with minimum allowable surface pressure =', Vgrid%psmin
       endif
       if (lverbose > 1) then
           write (logunit,*) 'INPUT VALUES:'
           write (logunit,*) '  eta =', Vgrid%eta
           write (logunit,*) '  peta =', Vgrid%peta
           write (logunit,*) 'OTHER VALUES:'
           write (logunit,*) '  deta =', Vgrid%deta
           write (logunit,*) '  dpeta =', Vgrid%dpeta
           write (logunit,*) '  nplev =', Vgrid%nplev
       endif
   endif
