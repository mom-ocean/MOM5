...
subroutine ocean_model_data2D_get(OS,Ocean, name, array2D,isc,jsc)
  ...
  case('frazil')
     array2D(isc:iec,jsc:jec) = Ocean%frazil(isc:iec,jsc:jec)
  case('eta_t')
...
subroutine mom4_set_swheat_fr(swheat_fr_input)
  real, dimension(isc:,jsc:,:), intent(in)  :: swheat_fr_input
  integer :: l,i,j
  
  do l=1,nk
     do j=jsc,jec
        do i=isc,iec     
           sw_frac_zt(i,j,l) = swheat_fr_input(i,j,l)
        end do
     end do
  end do

  call mpp_update_domains(sw_frac_zt, &
       Domain%domain2d)
