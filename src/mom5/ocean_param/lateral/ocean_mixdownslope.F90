...
use ocean_util_mod,        only: write_timestamp, write_chksum_2d, write_chksum_3d, write_chksum_2d_int
...
subroutine watermass_diag(Time, Dens)
  ...
  if(id_neut_rho_mixdown > 0) then 
      used = send_data (id_neut_rho_mixdown,wrk2(:,:,:), Time%model_time,&
             rmask=Grd%tmask(:,:,:),                                     &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_wdian_rho_mixdown > 0) then 
       used = send_data (id_wdian_rho_mixdown, wrk3(:,:,:),&
              Time%model_time,rmask=Grd%tmask(:,:,:),      &
              is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_tform_rho_mixdown > 0) then 
       used = send_data (id_tform_rho_mixdown, wrk4(:,:,:),&
              Time%model_time,rmask=Grd%tmask(:,:,:),      &
              is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_neut_rho_mixdown_on_nrho > 0) then 
      nrho_work(:,:,:) = 0.0
      call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk2, nrho_work) 
      used = send_data (id_neut_rho_mixdown_on_nrho, nrho_work(:,:,:),&
      Time%model_time,                                                &
      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif

  if (id_wdian_rho_mixdown_on_nrho > 0) then 
     nrho_work(:,:,:) = 0.0
     call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk3, nrho_work) 
     used = send_data (id_wdian_rho_mixdown_on_nrho, nrho_work(:,:,:),&
          Time%model_time,                                            &
          is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif

  if (id_tform_rho_mixdown_on_nrho > 0) then 
     nrho_work(:,:,:) = 0.0
     call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk4, nrho_work) 
     used = send_data (id_tform_rho_mixdown_on_nrho, nrho_work(:,:,:),&
            Time%model_time,                                          &
            is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
  ...
  if(id_eta_tend_mixdown > 0 .or. id_eta_tend_mixdown_glob > 0) then
      ...
      do k=1,nk
         ...
         enddo
      enddo
      if(id_eta_tend_mixdown > 0) then 
          used = send_data (id_eta_tend_mixdown, eta_tend(:,:),&
  ...
  if(id_neut_temp_mixdown > 0) then 
      used = send_data (id_neut_temp_mixdown,wrk2(:,:,:), Time%model_time,&
             rmask=Grd%tmask(:,:,:),                                      &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_wdian_temp_mixdown > 0) then 
       used = send_data (id_wdian_temp_mixdown, wrk3(:,:,:),&
              Time%model_time,rmask=Grd%tmask(:,:,:),       &
              is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_tform_temp_mixdown > 0) then 
       used = send_data (id_tform_temp_mixdown, wrk4(:,:,:),&
              Time%model_time,rmask=Grd%tmask(:,:,:),       &
              is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_neut_temp_mixdown_on_nrho > 0) then 
      nrho_work(:,:,:) = 0.0
      call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk2, nrho_work) 
      used = send_data (id_neut_temp_mixdown_on_nrho, nrho_work(:,:,:),&
      Time%model_time,                                                 &
      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif

  if (id_wdian_temp_mixdown_on_nrho > 0) then 
     nrho_work(:,:,:) = 0.0
     call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk3, nrho_work) 
     used = send_data (id_wdian_temp_mixdown_on_nrho, nrho_work(:,:,:),&
          Time%model_time,                                             &
          is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif

  if (id_tform_temp_mixdown_on_nrho > 0) then 
     nrho_work(:,:,:) = 0.0
     call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk4, nrho_work) 
     used = send_data (id_tform_temp_mixdown_on_nrho, nrho_work(:,:,:),&
            Time%model_time,                                           &
            is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
  ...
  if(id_neut_salt_mixdown > 0) then 
      used = send_data (id_neut_salt_mixdown,wrk2(:,:,:), Time%model_time,&
             rmask=Grd%tmask(:,:,:),                                      &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_wdian_salt_mixdown > 0) then 
       used = send_data (id_wdian_salt_mixdown, wrk3(:,:,:),&
              Time%model_time,rmask=Grd%tmask(:,:,:),       &
              is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_tform_salt_mixdown > 0) then 
       used = send_data (id_tform_salt_mixdown, wrk4(:,:,:),&
              Time%model_time,rmask=Grd%tmask(:,:,:),       &
              is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 

  if (id_neut_salt_mixdown_on_nrho > 0) then 
      nrho_work(:,:,:) = 0.0
      call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk2, nrho_work) 
      used = send_data (id_neut_salt_mixdown_on_nrho, nrho_work(:,:,:),&
      Time%model_time,                                                 &
      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif

  if (id_wdian_salt_mixdown_on_nrho > 0) then 
     nrho_work(:,:,:) = 0.0
     call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk3, nrho_work) 
     used = send_data (id_wdian_salt_mixdown_on_nrho, nrho_work(:,:,:),&
          Time%model_time,                                             &
          is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif

  if (id_tform_salt_mixdown_on_nrho > 0) then 
     nrho_work(:,:,:) = 0.0
     call rebin_onto_rho (Dens%neutralrho_bounds, Dens%neutralrho, wrk4, nrho_work) 
     used = send_data (id_tform_salt_mixdown_on_nrho, nrho_work(:,:,:),&
            Time%model_time,                                           &
            is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
